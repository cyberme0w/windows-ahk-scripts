; This script improves Windows' Virtual Desktop functionality.
; By keeping the current virtual desktop's ID in memory, AHK manages
; each swap between desktops - what that means is, that it's now possible to
; press Win+$number and you'll be transported to the $number-th desktop
; Similar to i3 or other Linux WMs.
...
switchDesktopByNumber(targetDesktop) {
    global CurrentDesktop, DesktopCount
    ; Re-generate the list of desktops and where we fit in that. We do this
    ; because the user may have switched desktops via some other means than the
    ; script.
    mapDesktopsFromRegistry()
